...
export fn main() noreturn {
    ...
    // 2. Perform mGBA handshake by writing 0xC0DE
    REG_DEBUG_ENABLE.* = MGBA_ENABLE_MAGIC;

    // 3. Read back value after handshake (0x1EA0 vs 0x0000 / open bus)
    ...
    if (readback_val == MGBA_RESPONSE_MAGIC) {
        hal.mgba.log.write(.info, "Result: Matched MGBA_RESPONSE_MAGIC (0x1EA0)");
    } else {
        const msg2 = std.fmt.bufPrint(
            &hal.mgba.log.format_buf,
            "Result: Open bus / unhandled readback (expected 0x1EA0, got 0x{X:0>4})",
            .{readback_val},
        ) catch unreachable;
